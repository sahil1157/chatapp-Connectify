...
const registerUser = async (req, res, next) => {
  ...
  try {
    ...
    if (createNewUser) res.status(200).json({ message: "Success" });
  } catch (error) {
    next({ message: error });
  }
};
...
const authUser = async (req, res, next) => {
  ...
  try {
    ...
    const getHashedPassword = await bcrypt.compare(
      password,
      findEmail.password
    );
    if (!getHashedPassword) {
      return res
        .status(400)
        .json({ valid: false, message: "Invalid password" });
    }

    // generating userToken
    const accessToken = jwt.sign({ email: email }, process.env.secretToken, {
      expiresIn: "2d",
    });
    const refreshToken = jwt.sign({ email: email }, process.env.secretToken, {
      expiresIn: "15d",
    });
...
const getAllUsers = async (req, res, next) => {
  ...
  try {
    ...
    const myDetails = await User.findById(myId).select(
      "avatar.url firstname _id lastname"
    );
    ...
    const chatsWithLatestMessages = userChats.map((x) => {
      const membersWithoutMe = x.members.find(
    ...
    res.status(200).json({
      ...
      myId,
      myDetails,
      latestMessages: chatsWithLatestMessages,
    });
  } catch (error) {
    next(error);
  }
